...
const TOOLS = [
  ...
  {
    name: "get_locations",
    description: "Fetch all locations from the FanHub API",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "get_location_by_id",
    description: "Fetch a single location by ID from the FanHub API",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Create a new location via the FanHub API",
    inputSchema: {
      type: "object",
      properties: {
        showId: { type: "number", description: "Show ID (1 for Breaking Bad)" },
        ...
        significance: {
          type: "string",
          description: "Story significance of the location",
        },
      },
      required: ["showId", "name", "description", "significance"],
    },
  },
];
  ...
  if (method === "tools/call") {
    ...
    try {
      ...
      switch (tool) {
        ...
        // Locations
        case "get_locations":
          data = await getJson("/api/locations");
          break;
        case "get_location_by_id":
          data = await getJson(`/api/locations/${args.id}`);
          break;
        case "create_location":
          data = await postJson("/api/locations", args);
          break;
